...
> In this chapter we'll use the `t.test()` function for fitting one and two sample t-test models with one categorical predictor, which will take the following form: 
...
## Importing {-}
For this chapter we'll be using three datasets: **rmr.xlsx** for the two sample indepedent t-test, **lactate_threshold.xlsx** for the two sample dependent t-test, and the third dataset for the one sample t-test will be created in R. 

First we'll create the data for the one sample t-test:
...
The code above creates a random normal distribution of 100 samples with a mean of 54.3 and a standard deviation of 5.3, which are both arbitrary values. 
...
The dataset for the two sample indepdentdent t-test contains resting metabolic rate values for several males and females which was collected using a metabolic cart.
...
### Data for Two Sample Independent {-}
We'll be comparing the resting metabolic rate of males and females for the independent t-test, so we need to filter the data to include only males or females, but not both. There are countless ways to do this, and one way is to split the `data2` object into a `data_M` object, which only contains data for the males, and a `data_F` object, which only contains data for the females. The `filter()` function comes from the `dplyr` package, so make sure you've loaded that library into your workspace.
