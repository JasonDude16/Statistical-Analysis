...
Or you can make a boxplot with the `boxplot()` function: 

```{r out.height= "70%", out.width= "70%"}
par(mfrow = c(1, 2))
boxplot(data$TotalKg)
boxplot(data$BodyweightKg)
```
...
summary(data[c(9,26)])
```
Notice how the summary also includes the `NA` count, which is a really nice feature.
...
### Untransformed Model {-} 
$$y_i\ =\ \beta_0\ +\ \beta_1\left(x_i\right)\ +\ e_i$$
The equation above represents the untransformed, simple regression model with one continuous predictor. In the `lm()` function, the dependent variable is listed first, followed by a tilda, `~`, and then the independent variable(s).
