...
library(ggplot2)
set.seed(123)
ggplot(tibble::tibble(rnorm = rnorm(100)), aes(x = rnorm)) +
    geom_density(tibble::tibble(rnorm = rnorm(100, mean = 2)), 
                 mapping = aes(x = rnorm), 
                 fill = "blue", 
                 alpha = .7) +
...
The `ggplot2` package is simply a collection of R code and functions, created by an R user, that allows you to make highly customizable plots like the one above with without too much difficult. 
...
## What's a Library? {-}
Once you install a package it becomes part of your "library". Your library is a collection of all the packages that are installed on your computer. If you click on the "packages" tab in the lower right pane, you can see a list of all the packages that are currently installed. This is your library. Notice how the header even says "System Library". 
